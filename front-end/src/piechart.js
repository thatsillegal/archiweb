...
function createPieChart(data) {
  ...
  console.log(lut.getColor(cnt/handle.length));
  data.forEach((e) => {
...
function create() {
  ...
  for (let data of window.piedata) {
    ...
    let end = new THREE.Vector3(radius * Math.cos(cur + angle) , radius * Math.sin(cur + angle));
    const lg = new THREE.BufferGeometry();
    const lm = new THREE.LineBasicMaterial({color: 0x000000});
    lg.setFromPoints([start, end]);
...
function main() {
  ...
  viewport.setCameraPosition([200, 0, 1000], [200, 0, 0])
